...
@Component({
    selector: 'jhi-position-linkder-delete-dialog',
    templateUrl: './position-linkder-delete-dialog.component.html'
})
export class PositionLinkderDeleteDialogComponent {
...
@Component({
    selector: 'jhi-position-linkder-delete-popup',
    template: ''
})
export class PositionLinkderDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ position }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(PositionLinkderDeleteDialogComponent as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
