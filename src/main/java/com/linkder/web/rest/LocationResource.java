...
public class LocationResource {
    ...
    public List<Location> getAllLocations(@RequestParam(required = false) String filter) {
        if ("candidate-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where candidate is null");
            return locationService.findAllWhereCandidateIsNull();
        }
        if ("company-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where company is null");
            return locationService.findAllWhereCompanyIsNull();
        }
        if ("project-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where project is null");
            return locationService.findAllWhereProjectIsNull();
        }
        if ("position-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where position is null");
            return locationService.findAllWherePositionIsNull();
        }
